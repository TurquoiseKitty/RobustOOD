"""给slides补充图片: 说明不考虑covariate和cluster-based method方法在toy example上面的局限性"""
...
num_train_samples_list = [1000]

alg_names = ["Ellipsoid","K-Means"]
legend_alg_names = ["Without covariates","K-Means"]
...
fig, axs = plt.subplots(2, 1, figsize=(10,10),sharex='col',sharey='row', gridspec_kw={'height_ratios': [3, 1]})
...
for i in range(len(num_train_samples_list)):
    ...
    for alg_idx,alg_name in enumerate(alg_names):
        line, = ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,1],color=colors[alg_idx],label=legend_alg_names[alg_idx],linewidth=3,linestyle=linestyle_list[alg_idx])
        ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,2],color=colors[alg_idx],linewidth=3,linestyle=linestyle_list[alg_idx])
        #legend_handles_row1.append(line)
    
    ax.legend(fontsize=20)
    ax.scatter(covs_train[:,0],c_train,color="black",s=5,zorder=10)
    ...
    ax.set_xlabel('$z$',fontsize=20)
    ax.set_title("RO solution", fontsize=20)
    #if i==0:
...
fig.savefig("toy/slides1.png", dpi=300, bbox_inches='tight')
fig.savefig("toy/slides1.pdf", bbox_inches='tight')
