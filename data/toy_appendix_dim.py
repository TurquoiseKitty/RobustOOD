...
fig, axs = plt.subplots(2, 3, figsize=(18,7.5),sharex='col',sharey='row', gridspec_kw={'height_ratios': [2, 1]})
...
for i in range(len(dim_covs_list)):
    ...
    ax.set_title("T="+str(num_train_samples)+", d="+str(dim_covs_display),fontsize=20)
    for alg_idx,alg_name in enumerate(alg_names):
        line, = ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,1],color=colors[alg_idx],label=alg_name)
...
fig.savefig("toy/dim.png", dpi=300, bbox_inches='tight')
fig.savefig("toy/dim.pdf", bbox_inches='tight')
