"""Insights from the analysis: different """
...
ax = axs[0]
ax.set_title('Uncertainty Sets',fontsize=20)
# set ax size
...
for alg_idx,alg_name in enumerate(alg_name_list):
    ...
    line, =ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,1],color=colors[alg_idx],label=alg_name,linestyle = linestyles[alg_idx],linewidth=3)
    ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,2],color=colors[alg_idx],linestyle = linestyles[alg_idx],linewidth=3)

    legend_handles_row1.append(line)
    ax.scatter(covs_train[:,0],c_train,color="black",s=2)
...
fig.savefig("toy/slides_diff_f.png",bbox_inches='tight')
fig.savefig("toy/slides_diff_f.pdf",bbox_inches='tight')
...
h_model_name_box_list = ["Linear","GBR","MLP"]
...
for h_model_name_box in h_model_name_box_list:
    # go to PTC dir
    os.chdir(PTC_dir)
...
for h_model_name_box in h_model_name_box_list:
    ...
    prefix = "LUQ/"+f_model_name+"/quantile/"+h_model_name_box+"/"+str(alpha)+"/"
    filename = train_dir+prefix
    if h_model_name_box=="MLP":
        h_model_name_box = "NN"
    if os.path.exists(filename):
        #alg_name_lists[-1].append("PTC"+"-box-"+f_model_name+"-"+h_model_name_box)
        alg_name_list.append(h_model_name_box)
        coverage = in_box(train_dir+prefix,c_test)
        LB,UB = get_LB_UB_from_coverage(coverage,c_test)
        LB_list.append(LB)
        UB_list.append(UB)
...
for alg_idx,alg_name in enumerate(alg_name_list):
    ...
    line, = ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,1],color=colors[alg_idx],label=alg_name,linestyle=linestyles[alg_idx],zorder=zorders[alg_idx],linewidth=3)
    ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,2],color=colors[alg_idx],linestyle = linestyles[alg_idx],zorder=zorders[alg_idx],linewidth=3)
...
fig.savefig("toy/slides_diff_h.png",bbox_inches='tight')
fig.savefig("toy/slides_diff_h.pdf",bbox_inches='tight')
