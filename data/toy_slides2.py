"""给slides补充图片：对比Ellipsoid方法、kNN、k-Means的优缺点：kNN无法对抗噪声，k-Means在adaptivity上差一点"""
...
alg_names = ["Ellipsoid","kNN","K-Means"]
legend_alg_names = ["Ellipsoid","kNN","K-Means"]

colors = ["purple","green","orange","red","blue"]

linestyles = ["dashed","solid","solid"] #["solid","dotted","dashed","dashdot"]
...
fig, axs = plt.subplots(2, len(dim_covs_list), figsize=(15,7.5),sharex='col',sharey='row', gridspec_kw={'height_ratios': [3, 1]})
...
for i in range(len(dim_covs_list)):
    ...
    for alg_idx,alg_name in enumerate(alg_names):
        line, = ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,1],color=colors[alg_idx],label=alg_name,linewidth=3,linestyle=linestyles[alg_idx])
        ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,2],color=colors[alg_idx],linewidth=3,linestyle=linestyles[alg_idx])
...
legend1 = fig.legend(legend_handles_row1,legend_alg_names,loc='upper center',ncol=6,fontsize=20, bbox_to_anchor=(0.5, 0.93))
#lt.subplots_adjust(top=0.8)  
# legend1.set_bbox_to_anchor((0.5, 1.1))
legend2 = fig.legend(legend_handles_row2,["$x=-1$","$x=0$","$x=1$"],loc='lower center', bbox_to_anchor=(0.5,0.02), ncol=3,fontsize=20)
#plt.subplots_adjust(bottom=0.2)  
# legend2.set_bbox_to_anchor((0.5, -0.15))
# fig.tight_layout()

fig.savefig("toy/slides2.png", dpi=300, bbox_inches='tight')
fig.savefig("toy/slides2.pdf", bbox_inches='tight')
