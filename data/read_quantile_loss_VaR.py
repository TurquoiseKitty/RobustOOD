...
def read_quantile_loss_Var(train_dir,test_dir,alpha,f_model_name,h_model_name_list,task_name,box_or_ellipsoid):
    mse_list = []
    Var_list = []
    ...
    for h_model_name_box in h_model_name_list:
        ...
        if box_or_ellipsoid=="box":
            result_dir = train_dir+"/"+f_model_name+"/quantile/"+h_model_name_box+"/"+str(alpha)+"/"
        elif box_or_ellipsoid=="ellipsoid":
            result_dir = train_dir+"/"+f_model_name+"/norm/"+h_model_name_box+"/"+str(alpha)+"/"


        mse_file = result_dir+"quantile_loss_test.txt"    
        ...
        if task_name=="shortest_path":
            # load csv
            x_sol = pd.read_csv(result_dir+"x_sol.csv",header=None).to_numpy()
        elif task_name=="knapsack":
            # load npy
            x_sol = np.load(result_dir+"x_sol.npy")
