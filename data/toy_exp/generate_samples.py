...
def generate_covs(num_train_samples):
    ...
    covs = np.random.uniform(low = 0, high = 1, size=(num_train_samples,1))
    return covs

def generate_random_res(num_train_samples):
    res = np.random.uniform(low = 0, high = 0.2, size=(num_train_samples,1))
    return res

def generate_c(covs,eps,const):
    c = 2/(1+np.exp(covs[:,0]+eps[:,0]))-const
...
def generate_one_dim_samples(deg,num_train_samples,const,num_test_samples,random_seed=0):
    ...
    # generate test data
    cov_test = np.linspace(0,1,num_test_samples).reshape(-1,1)
...
if __name__=="__main__":
    num_train_samples_list = [50,100,200,500,1000]
    deg = 1
    dim_covs_list = [2]
