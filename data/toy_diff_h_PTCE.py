...
for h_model_name_box in h_model_name_box_list:
    ...
    subprocess.call(["python","train_2norm_h.py","--task_name",task_name,"--dim_covs",str(dim_covs),"--num_train_samples",str(num_train_samples),"--deg",str(deg),"--f_model_name",f_model_name,"--h_model_name",h_model_name_box,"--alpha",str(alpha)])
# go back to cur_dir
os.chdir(cur_dir)
...
for h_model_name_box in h_model_name_box_list:
    ...
    prefix = "LUQ/"+f_model_name+"/norm/"+h_model_name_box+"/"+str(alpha)+"/"
...
for alg_idx,alg_name in enumerate(alg_name_list):
    ...
    line, = ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,1],color=colors[alg_idx],label=alg_name,linestyle=linestyles[alg_idx],zorder=zorders[alg_idx])
    ax.plot(alg_np_list[alg_idx][:,0],alg_np_list[alg_idx][:,2],color=colors[alg_idx],linestyle = linestyles[alg_idx],zorder=zorders[alg_idx])
...
fig.savefig("toy/diff_h_PTCE.png",bbox_inches='tight')
fig.savefig("toy/diff_h_PTCE.pdf",bbox_inches='tight')
