...
# from optim.deepSVDD_trainer_AE_AC_cen_proj import DeepSVDDTrainer
from optim.deepSVDD_trainer_AE_AC import DeepSVDDTrainer
...
class DeepSVDD(object):
    ...
    def set_network(self, net_name,main_dim,side_dim,out_dim,beta,lmbda,n_class):
        ...
        self.encoder,self.decoder,self.soft_KMeans,self.net_main = build_network(net_name,main_dim,encode_input_size=side_dim,out_size=out_dim,beta=beta,lmbda=lmbda,n_class=n_class)


    def train(self, dataset: BaseADDataset, n_class: int = 1 ,optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 50,
        ...
        # Get the model
        self.net = self.trainer.train(dataset, self.encoder,self.decoder,self.soft_KMeans,self.net_main)
        self.R = self.trainer.R#.cpu().data.numpy()  # get float
        self.c = self.trainer.c#.cpu().data.numpy().tolist()  # get list
        self.cov_mat = self.trainer.cov_mat
    ...
    def pretrain(self, dataset: BaseADDataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 100,
        ...
        self.ae_net = Sequential(self.encoder, self.decoder)
    ...
    def save_model(self, export_model, save_ae=True):
        ...
        file_gang_list = export_model.split('//')

        [fname,fename] = os.path.split(export_model)
        print(fname,fename)
        if not os.path.exists(fname):
            os.makedirs(fname)

        # if the "save_model" is not called after a pretraining process
        if not save_ae:
            net_dict = self.net.state_dict()
            ae_net_dict = self.ae_net.state_dict() 
            torch.save({'R': self.R,
    ...
    def load_model(self, model_path, load_ae=False):
        ...
        # if only load ae model
        if load_ae:
            model_dict = torch.load(model_path)

            self.ae_net = Sequential(self.encoder, self.decoder)
            self.ae_net.load_state_dict(model_dict['ae_net_dict'])
        # else, load the whole model
        else:
            self.R = model_dict['R']
            self.c = model_dict['c']
