...
'''This algorithm should be called only after calling kNN/main_deep_kmeans.py'''
...
load_dir = train_dir + "DCC/"+str(n_cluster)+"/"
...
save_dir = train_dir+"DCC_notDeep/"+str(n_cluster)+"/"+str(alpha)+"/"
...
if not os.path.exists(save_dir+"x_sol.npy") and not os.path.exists(save_dir+"x_sol.csv"):
    ...
    if os.path.exists(save_dir+"covss.npy") and os.path.exists(save_dir+"mus.npy") and os.path.exists(save_dir+"Rs.npy"):
        ...
        # load train_assignment.npy from load_dir
        train_assignment = np.load(load_dir+"train_assignments.npy")
        # load test_assignment.npy from load_dir
        test_assignment = np.load(load_dir+"test_assignments.npy")

        
        labels = np.max(train_assignment,axis=1).astype(int)
        labels_test = np.max(test_assignment,axis=1).astype(int)
        ...
        for k in range(n_cluster):
            if np.sum(labels==k)==0:
                # if no point in this cluster, set covs, mus, Rs to be the cluster covering alpha of all the training data
                covs_clusters[k,:,:] = np.cov(c_train.transpose())
                mus_clusters[k,:] = np.mean(c_train,axis=0)
                c_points = c_train
            else:
